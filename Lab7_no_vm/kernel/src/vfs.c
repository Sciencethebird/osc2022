...
int vfs_find_vnode(struct vnode** target, const char* pathname) {
  ...
  while (component_name && *component_name != '\0') {
    ...
    if (!found) {
      printf("[vfs_find_vnode] error: No such file or directory: %s\n", pathname);
      free(pathname_);
      return 0;
    }
...
struct file* vfs_open(const char* pathname, int flags) {
  ...
  //printf("file_found: %d\n", dir->v_ops->lookup(dir, &target, filename));
  if (flags == O_CREAT) {
    if (!file_found) {
...
void vfs_list(const char* pathname) {
  struct vnode *dir;
  if(vfs_find_vnode(&dir, pathname) == 1){
    dir->f_ops->list(dir);
  } else {
    printf("[vfs_list] no such file or directory!\n");
  }
}
...
int vfs_mount(const char* device, const char* mountpoint,
  ...
  if (!dir_found) {
    printf("[vfs_mount][Error] Directory does not exist: %s\n", mountpoint);
    return 0;
  }
  if (target->mount) {
    printf("[vfs_mount][Error] This directory has been mounted: %s\n", mountpoint);
    return 0;
  }
  ...
  if(parent_found == 0){
    printf("[vfs_mouont][Error] parent node not found\n");
  }
  free(pathname_);
