...
static void pin_setup() {
  // select gpio function for each gpio pings
  // GPIO_GPFSEL4 bit 29-27 is for 49 and 100 is for SD0_CMD
  // see https://elinux.org/RPi_BCM2835_GPIOs for GPIO alternative functions
  set(GPIO_GPFSEL4, 0x24000000); // pin 49 uses SD0_CMD, pint 48 uses SD0_CLK
  set(GPIO_GPFSEL5, 0x924); // GPIO 50-53 all for SD0 functions
  set(GPIO_GPPUD, 0);
  delay(15000);
...
void readblock(int block_idx, void* buf) {
  ...
  do {
    ...
    for (int i = 0; i < 128; ++i) { // 16 bit (4 Byte) each time
      wait_fifo();
